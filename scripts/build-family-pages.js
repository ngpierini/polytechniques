...
function hubHtml(groups) {
  ...
  // The structure map. Its data lives in structure-map.js, generated by
  // scripts/build-structure-map.js and checked in CI, so this markup is inert
  // until that has run.
  h += '  <div class="card">\n    <h3>The library as a map</h3>\n';
  h += '    <p>Every structure in the library placed next to the ones it chemically resembles. Nothing is predicted here and no property was used to build it: each repeat unit becomes thirty descriptors read straight off its atoms and bonds &ndash; what elements it contains, how branched it is, whether it is aromatic, what links its backbone &ndash; and those are projected onto two dimensions. Polymers that sit together sit together because they are built alike.</p>\n';
  h += '    <div class="smap-wrap">\n';
  h += '      <canvas id="smap" width="900" height="560" aria-label="A map of the polymer library, each point one structure, coloured by family"></canvas>\n';
  h += '      <p id="smap-readout" class="guide-note smap-readout" role="status" aria-live="polite">Hover a point to name it. Click to search for it.</p>\n';
  h += '      <div id="smap-key" class="smap-key"></div>\n';
  h += '    </div>\n';
  h += '    <p class="guide-note">How well does it work? Take each structure&rsquo;s eight nearest neighbours on the map and ask how often they come from the same family: <strong>45&nbsp;%</strong>, against <strong>13&nbsp;%</strong> if the points were scattered at random &ndash; a 3.4&times; lift. In the full thirty-dimensional descriptor space that figure is 54&nbsp;%, so flattening it to a picture costs about a fifth of the structure it found. That gap is the honest limit of any two-dimensional map, and it is why this one is for browsing rather than for drawing conclusions from a distance.</p>\n';
  h += '    <p class="guide-note">The families overlap, and that is chemistry rather than a failure of the method: an acrylate and a methacrylate differ by one methyl group, a polyester and a polyamide by a single atom in the linkage. Where the map does separate them cleanly, it is telling you the backbones really are different.</p>\n';
  h += '  </div>\n\n';
  h += '  <div class="card">\n    <h3>Families</h3>\n';
  groups.forEach(g => {
